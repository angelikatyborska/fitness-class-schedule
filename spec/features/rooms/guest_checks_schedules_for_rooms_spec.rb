...
feature 'Guest check schedules for rooms' do
  ...
  given!(:small_schedule_item_this_week) {
    item = build(:schedule_item, room: small_room, activity: 'ABT', start: Time.zone.parse('8:05'), capacity: 6 )
    item.save(validate: false)
    item
  }
  given!(:small_schedule_item_next_week) {
    item = build(:schedule_item, room: small_room, activity: 'Step', start: Time.zone.parse('8:15') + 7.days, capacity: 6 )
    item.save(validate: false)
    item
  }
  given!(:big_schedule_item_this_week) {
    item = build(:schedule_item, room: big_room, activity: 'TBC', start: Time.zone.parse('9:05'), capacity: 20)
    item.save(validate: false)
    item
  }
  given!(:big_schedule_item_next_week) {
    item = build(:schedule_item, room: big_room, activity: 'Step', start: Time.zone.parse('9:15') + 7.days, capacity: 20)
    item.save(validate: false)
    item
  }
