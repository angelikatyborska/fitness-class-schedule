FactoryGirl.define do
  factory :schedule_item do
    ...
    factory :schedule_item_this_week do
      start { Faker::Time.between(Time.zone.now.beginning_of_week, Time.zone.now.beginning_of_week + 6.day).beginning_of_day + (Faker::Number.between(1, 80) * 15).minutes }
    end

    factory :schedule_item_next_week do
      start { Faker::Time.between(Time.zone.now.beginning_of_week + 7.days, Time.zone.now.beginning_of_week + 13.day).beginning_of_day + (Faker::Number.between(1, 80) * 15).minutes }
    end
  end
end
